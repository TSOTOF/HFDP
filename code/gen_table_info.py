...
import numpy as np
import os
df_table = pd.DataFrame(np.full([1,8],np.nan)) #分时逻辑库数据表代码
# tablename | 数据表名称
...
# mkt       | 数据表对应市场
# code      | 数据表对应证券代码
df_table.columns = ['tablename','format','pure','type','freq','year','mkt','code']
## 建立逻辑库表格
...
for i in range(len(df_filename)): 
    ...
    table_ = pd.DataFrame(df_.apply(info,args = (format,year),axis = 1,result_type="expand"))
    table_.columns = ['tablename','format','pure','type','freq','year','mkt','code']
